...
public class Withdrawal extends JFrame implements ActionListener {
    ...
    Withdrawal(String pinnumber) {
        ...
        text.setBounds(320, 160, 400, 20);
        image.add(text);
        
        amount = new JTextField();
        amount.setFont(new Font("Raleway", Font.BOLD, 22));
        amount.setBounds(310, 200, 320, 25);
        image.add(amount);
        
        withdraw = new JButton("Withdraw");
        withdraw.setBounds(480, 260, 150, 30);
        withdraw.addActionListener(this);
        image.add(withdraw);
        
        back = new JButton("Back");
        back.setBounds(480, 300, 150, 30);
    ...
    public void actionPerformed(ActionEvent qw){
        if(qw.getSource() == withdraw) {
            String number = amount.getText();
            Date date = new Date();
            if(number.equals("")){
                JOptionPane.showMessageDialog(null, "Please enter the amount you want to withdraw");
            } else {  //when user entered amount
                try{
                    Conn conn = new Conn();
                    String query = "insert into bank values('"+pinnumber+"', '"+date+"', 'Withdrawal', '"+number+"')";
                    conn.st.executeUpdate(query); //depositing in actual
